...
Voting is done in a [xonotic.org Matrix room](/chat) using [PyTIBot](https://github.com/DefaultUser/PyTIBot) which also [publishes the results](https://xonotic.org/balancevotes).  Changes to Council membership are decided by the votes of existing members.  Any member may call a membership or gameplay change vote.  Gameplay votes run for 30 days, membership votes run for 10 days, and voting will conclude early if a supermajority occurs.  [Xonotic Team](/team) members are always considered Council members, and the Team may override Council votes.
